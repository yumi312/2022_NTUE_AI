...
app.title = 'COVID-19 Global Panel (全球即時資訊）'
...
df_worldwide = pd.read_csv(
    'https://raw.githubusercontent.com/raffg/covid-19/master/data/df_worldwide.csv')  # not work?
# 將percentage轉換成str的資料型態: astype
df_worldwide['percentage'] = df_worldwide['percentage'].astype(str)
df_worldwide['date'] = pd.to_datetime(df_worldwide['date'])

# define the "data last updated" date
...
def confirmed(view):
    ...
    return {
        'data': [{'type': 'indicator',
                  ...
                  'domain': {'y': [0, 1], 'x': [0, 1]}}],
        'layout': go.Layout(
            title={'text': "累計確診數 CUMULATIVE CONFIRMED"},
...
def active(view):
    ...
    return {
        'data': [{'type': 'indicator',
                  ...
                  'domain': {'y': [0, 1], 'x': [0, 1]}}],
        'layout': go.Layout(
            title={'text': "治療中 CURRENTLY ACTIVE"},
            font=dict(color=dash_colors['red']),
            paper_bgcolor=dash_colors['background'],
            plot_bgcolor=dash_colors['background'],
            height=200
...
def recovered(view):
    ...
    return {
        'data': [{'type': 'indicator',
                  ...
                  'domain': {'y': [0, 1], 'x': [0, 1]}}],
        'layout': go.Layout(
            title={'text': "已復原 RECOVERED CASES"},
            font=dict(color=dash_colors['red']),
            paper_bgcolor=dash_colors['background'],
            plot_bgcolor=dash_colors['background'],
            height=200
...
def set_countries_value(view, available_options):
    ...
    '''
    if view == 'Worldwide':
        return ['US', 'India', 'United Kingdom', 'Spain', 'France', 'Russia', 'Brazil', 'Belgium', 'Peru']
    elif view == 'United States':
        ...
        return ['US', 'India', 'Italy', 'United Kingdom', 'Spain', 'France', 'Germany', 'Russia']
...
def active_countries(view, countries, column, population):
    ...
    for country in countries:
        if population == 'absolute':
            y_data = df[df['Country/Region'] ==
                        country].groupby('date')[column].sum()
            recovered = df[df['Country/Region'] ==
                           'Recovered'].groupby('date')[column].sum()
        elif population == 'percent':
    ...
    if column == 'Recovered':
        traces.append(go.Scatter(
            x=df[df['Country/Region'] ==
                 'Recovered'].groupby('date')['date'].first(),
            y=recovered,
            hovertemplate=hover,
            name='Unidentified',
            mode='lines'))
        ...
        'data': [
            go.Scattergeo(
                ...
                marker=dict(reversescale=False,
                            ...
                            cmax=100,
                            colorbar=dict(
                                title="％ of<br>cases occurring in<br>the previous week",
                                thickness=30)
                            )
            )
        ],
        'layout': go.Layout(
            title='確診數(圓圈大小)<br>與前一週比較(color)',
...
def trajectory(view, date_index):
    ...
    df['previous_week'] = df.groupby(
        ['Country/Region'])['Confirmed'].shift(7, fill_value=0)
    ...
    countries_full = df.groupby(by='Country/Region', as_index=False)['Confirmed'].max(
    ).sort_values(by='Confirmed', ascending=False)['Country/Region'].to_list()

    df = df[df['date'] <= date]

    countries = df.groupby(by='Country/Region', as_index=False)[
        'Confirmed'].max().sort_values(by='Confirmed', ascending=False)
    countries = countries[countries['Confirmed']
                          > threshold]['Country/Region'].to_list()
    ...
    return {
        'data': traces,
        'layout': go.Layout(
            title='Trajectory of Cases<br>({} 確診數多於 {:,} 人)'.format(
                scope, threshold),
            xaxis_type="log",
            yaxis_type="log",
...
app.layout = html.Div(style={'backgroundColor': dash_colors['background']}, children=[
    html.H1(children='COVID-19 Global Panel (전 세계에 거주／全球即時資訊／世界中に住んでいる／）',
            style={
                'textAlign': 'center',
    ...
    html.Div(children='Data updated 資料更新 {} end-of-day'.format(update), style={
        'textAlign': 'center',
        'color': dash_colors['text']
    }),

    html.H6(children='focus on：選取範圍(全球／美國／歐洲／中國)', style={
        'textAlign': 'center',
        'color': dash_colors['text']
    }),
    ...
    html.Div(dcc.RadioItems(id='global_format',
                            options=[{'label': i, 'value': i} for i in [
                                'Worldwide(全球)', 'United States(美國)', 'Europe(歐洲)', 'China(中國)']],
                            value='Worldwide',
    ...
    html.Div(dcc.Markdown('Display data as total or as relative to population(數字呈現之級數):'),
             style={
        'textAlign': 'center',
        'color': dash_colors['text'],
        'width': '100%',
    ...
    html.Div(dcc.RadioItems(id='population_select',
                            options=[{'label': 'Total values(總數)', 'value': 'absolute'},
                                     {'label': 'Values per 100,000 of population(每十萬人數字)', 'value': 'percent'}],
                            value='absolute',
                            labelStyle={'float': 'center',
                                        'display': 'inline-block'},
                            style={'textAlign': 'center',
    ...
    html.Div(html.Div(dcc.Slider(id='date_slider',
                                 min=list(range(len(df_worldwide['date'].unique())))[
                                     0],
                                 max=list(
                                     range(len(df_worldwide['date'].unique())))[-1],
                                 value=list(
                                     range(len(df_worldwide['date'].unique())))[-1],
